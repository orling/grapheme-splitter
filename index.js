...
export const nextBreak = (str, index) => {
  if (index === undefined)
    index = 0;
  if (index < 0)
    return 0;
  ...
  const prev = getGraphemeBreakProperty(str.codePointAt(index));
  const mid = []
  for (var i = index + 1; i < str.length; i++) {
    ...
    var next = getGraphemeBreakProperty(str.codePointAt(i));
...
export const graphemeIterator = str => ({
  *[Symbol.iterator]() {
    let index = 0;
    let brk;
    while((brk = nextBreak(str, index)) < str.length){
      yield str.slice(index, brk);
      index = brk;
    }
    if (index < str.length)
      yield str.slice(index);
  }
});

export const splitGraphemes = str => [...graphemeIterator(str)];
